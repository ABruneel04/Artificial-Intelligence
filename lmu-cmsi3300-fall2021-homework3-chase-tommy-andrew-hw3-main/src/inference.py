...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        We started off by checking to see if they were in jail. Then, if its
        not in jail we loop through the legal positions. At each position
        we get the true distance, put it into the model, and mutilpy the
        probability by our current belief to get our new belief. We then
        normalize it to get our final distribution.
        """
        ...
        # Finding the total evidence to normalize by and update beliefs
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        We loop through the old positions and apply our transition model, then
        sum up the probabilities that it is in the new positions. We then update
        our beliefs
        """
        ...
        for oldPos in self.legalPositions:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, oldPos))
            probOld = self.beliefs[oldPos]
            for newPos in newPosDist:
                oldProbLoc = allPossible[newPos]
                newProb = oldProbLoc + (probOld * newPosDist[newPos])
                allPossible[newPos] = newProb
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        To spread them out evenly, we took the number of particles and the
        number of spaces to find out how many we should put in each. We rounded
        down and then if there were any left we pick randomly to distribute them
        so that we are normally not clumping them in a given area.
        """

        self.beliefs = []
        totParticles = self.numParticles
        particlesPerPos = totParticles / len(self.legalPositions)
        for legalPos in self.legalPositions:
            particlesInPos = 0
            while particlesInPos < particlesPerPos:
                particlesInPos = particlesInPos + 1
                self.beliefs.append(legalPos)
                totParticles = totParticles - 1
        if totParticles > 0:
            while totParticles > 0:
                self.beliefs.append(random.choice(self.legalPositions))
                totParticles = totParticles - 1
    ...
    def observe(self, observation, gameState):
        ...
        We are checking for jail again in the same way. We then loop through
        our beliefs, getting the true distance, putting that into the model, and
        adding that weight to our running total. Once its done, assuming the
        weights were not zero then we sample from it.
        """
        noisyDistance = observation
        emissionModel = busters.getObservationDistribution(noisyDistance)
        pacmanPosition = gameState.getPacmanPosition()
        ...
        if allPossible.totalCount() == 0:
            ...
            self.beliefs = util.nSample(allPossible.values(), allPossible.keys(), self.numParticles)
    ...
    def elapseTime(self, gameState):
        ...
        We loop through every positin that a particle is in, and then get a
        transition model. We are looping through each new
        place the particle could be and assigning that a weight based on the model.
        Then we sample and update our beliefs.
        """
        allPossible = util.Counter()
        for particlePos in self.beliefs:
            newPosDist = self.getPositionDistribution(self.setGhostPosition(gameState, particlePos))
            for newPos in newPosDist:
                allPossible[newPos] = allPossible[newPos] + newPosDist[newPos]
        self.beliefs = []
        for particle in allPossible:
            self.beliefs.append(util.sample(allPossible))
    ...
    def getBeliefDistribution(self):
        ...
        We are looping through the positions of the particles and adding into
        a dictonary to create a tally of how many are in each spot.
        """

        beliefDistrib = util.Counter()
        for particle in self.beliefs:
            beliefDistrib[particle] = beliefDistrib[particle] + 1
        return beliefDistrib
