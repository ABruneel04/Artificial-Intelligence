...
class MazeClause:
    ...
    def __init__(self, props):
        ...
        self.valid = False
        for i in props:
            if self.get_prop(i[0]) is None or self.get_prop(i[0]) == i[1]:
                self.props[i[0]] = i[1]
            else:
                self.props.pop(i[0])
                self.valid = True
        ...
        # Might change as you're loading in the props
    ...
    def resolve(c1, c2):
        ...
        key = 0
        for i in c1.props:
            propList.append((i,c1.get_prop(i)))
        copyC1 = MazeClause(propList)
        propList=[]
        for i in c2.props:
            propList.append((i,c2.get_prop(i)))
        copyC2 = MazeClause(propList)
        propList=[]
        for i in copyC1.props:
            if copyC1.get_prop(i) != copyC2.get_prop(i) and copyC2.get_prop(i)!= None:
                copyC1.props.pop(i)
                copyC2.props.pop(i)
                key = 1
                break
        for i in copyC1.props:
            propList.append((i,copyC1.get_prop(i)))
        for i in copyC2.props:
            propList.append((i,copyC2.get_prop(i)))
#            if mc.is_valid and key == 1:
#                results.add(mc)
#                return results
        mc = MazeClause(propList)
        if key == 0:
            return results
        if mc.valid:
            return results
